...
// The CLI ships the entire skills/openjobs/ tree as its bundled `skill/`
// directory (generated by scripts/sync-cli-skill.mjs at pack time). Derive the
// expected packed paths from that single source of truth so this check fails
// if any skill file — including newly added references/ or scripts/ — stops
...
const cliSkillFiles = walkRelFiles(path.join(rootDir, "skills", "openjobs"))
  .map((rel) => `skill/${rel}`)
  .sort();

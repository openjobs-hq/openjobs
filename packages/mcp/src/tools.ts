...
export function createToolDefinitions(ctx: ToolContext): ToolDefinition[] {
  ...
  const readTools: ToolDefinition[] = [
    ...
    { name: "openjobs_get_checkout_session", description: "Read the status of a hosted checkout top-up session (created, pending, paid, credited, failed, expired).", inputSchema: schema({ sessionId: { type: "string" } }, ["sessionId"]), requiresAuth: true, handler: async (args) => run(() => client(ctx).wallet.getCheckoutSession(stringArg(args, "sessionId")!), ["openjobs_get_wallet_balance"]) },
  ...
  const writeTools: ToolDefinition[] = [
    ...
    { name: "openjobs_create_checkout_session", description: "Create a hosted checkout session to top up the agent's USDC ledger balance. Returns a checkoutUrl that a HUMAN must open in a browser to approve and pay (bank card, PayPal, Apple Pay, Google Pay, or stablecoins); no funds move until the human completes payment there. The ledger is credited automatically once payment settles.", inputSchema: schema({ amount: { type: "number", description: "USDC amount to top up (server-enforced bounds, default 1 to 10000)." }, currency: { enum: ["USDC"] }, returnUrl: { type: "string", description: "Optional relative path to suggest after payment completes." }, ...commonWriteProperties() }, ["amount"]), requiresAuth: true, mutates: true, handler: async (args) => { const blocked = requireConfirm(ctx, args, "create a hosted checkout payment link"); if (blocked) return blocked; const amount = numberArg(args, "amount"); if (amount === undefined || amount <= 0) return failure("OPENJOBS_INVALID_AMOUNT", "amount must be a positive number of USDC.", false); return run(() => client(ctx).wallet.createCheckoutSession({ amount, currency: "USDC", returnUrl: stringArg(args, "returnUrl", false) }, { idempotencyKey: stringArg(args, "idempotencyKey", false) }), ["openjobs_get_checkout_session", "openjobs_get_wallet_balance"]); } },
  ];
